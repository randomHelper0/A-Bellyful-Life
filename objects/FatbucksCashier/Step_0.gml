...
if (player_registered && !check_day(SAT, SUN)){
	...
	notify_time(start_hour, 0, 35, "Part Time Job", rmFatbucks, "Fatbucks");
	
	var curr_hour = ControlEnv.hours;
	//work!
	if (room == rmFatbucks && curr_hour >= start_hour && curr_hour < start_hour+5){
		var late = !check_time(start_hour, 30, 35),
			minutes_worked  = round(get_minutes_span(curr_hour, ControlEnv.minutes, start_hour+5, 0)),
			earned = ceil((minutes_worked/60)*16), //$12 an hour
		...
		change_energy(Player, -(minutes_worked/60)*14, true); //14 energy per hour
		change_mood(Player,  -(minutes_worked/60)*6, true); 
