function refresh_backpack() {
	with(ItemCard) {
	    ...
	    if (item.character == noone && !item.is_template)
	        instance_destroy();
	}
	...
	for (i = 0; i < instance_number(Item); i ++){
	    ...
	    if (!item.is_tool || item.is_template || item.in_fridge){
	        continue;
	    }
	...
	for (i = 0; i < instance_number(Item); i ++){
	    ...
	    item = instance_find(Item, i);
	    if (item.is_tool || item.in_stomach || item.in_bowels || item.is_template || item.in_fridge){
	        continue;
	    }
