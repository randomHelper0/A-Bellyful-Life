	...
	if( current_message != noone ) {
	    ...
	    for( i=0; i<current_word; i++ ){
	        ...
	        if( me[0] == "word" ) {
	            draw_set_color( me[3] );
	            draw_set_font( me[4] );
	            draw_text( x+padding_left+me[1], y+padding_top+me[2], string_hash_to_newline(me[5]) );
	        }
	    }
	    if( !pause ) {
	        if( current_word < ds_list_size( current_message ) ) {
	            ...
	            if( me[0] == "word" ) {
	                draw_set_color( me[3] );
	                draw_set_font( me[4] );
	                draw_text( x+padding_left+me[1], y+padding_top+me[2], string_hash_to_newline(string_copy( me[5], 1, floor( current_pos ) )) );
	            }
	        }
	    }
